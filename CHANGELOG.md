...
> **Scope:** this root CHANGELOG records the *release train* of the ecosystem (RFC + SDKs + integrations + contracts co-versioned starting at 1.0.0). Each publishable package additionally maintains its own `CHANGELOG.md` with granular detail.
...
- Co-versioning of the 9 packages (`@agentdid/sdk`, `agent-did-sdk`, `contracts`, `@agentdid/langchain`, `agent-did-langchain`, `agent-did-crewai`, `agent-did-semantic-kernel`, `agent-did-microsoft-agent-framework`, `agent-did-a2a`) under a single `v1.0.0` tag.
