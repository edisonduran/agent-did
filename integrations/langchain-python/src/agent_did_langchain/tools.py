...
def _build_get_current_identity_tool(runtime_identity: RuntimeIdentity, tool_prefix: str) -> StructuredTool:
    def get_current_identity() -> dict[str, Any]:
        try:
            return _serialize_snapshot(runtime_identity)
        except Exception as error:  # pragma: no cover - defensive boundary
            return _structured_error(error)

    return StructuredTool.from_function(
        func=get_current_identity,
        name=_with_prefix(tool_prefix, "get_current_identity"),
        description="Return the current Agent-DID identity attached to this LangChain agent.",
        args_schema=EmptyArgs,
    )


def _build_resolve_did_tool(runtime_identity: RuntimeIdentity, tool_prefix: str) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        ...
        name=_with_prefix(tool_prefix, "resolve_did"),
        description="Resolve an Agent-DID document. If no DID is provided, resolves the current agent DID.",
        args_schema=ResolveDidArgs,
    )


def _build_verify_signature_tool(runtime_identity: RuntimeIdentity, tool_prefix: str) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        ...
        name=_with_prefix(tool_prefix, "verify_signature"),
...
def _build_sign_payload_tool(
    agent_identity: AgentIdentity, runtime_identity: RuntimeIdentity, tool_prefix: str
) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        ...
        name=_with_prefix(tool_prefix, "sign_payload"),
...
def _build_sign_http_request_tool(
    ...
    return StructuredTool.from_function(
        ...
        name=_with_prefix(tool_prefix, "sign_http_request"),
        description="Create Agent-DID HTTP signature headers for an outbound request.",
        args_schema=SignHttpRequestArgs,
    )


def _build_rotate_keys_tool(runtime_identity: RuntimeIdentity, tool_prefix: str) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        ...
        name=_with_prefix(tool_prefix, "rotate_key"),
...
def _build_document_history_tool(runtime_identity: RuntimeIdentity, tool_prefix: str) -> StructuredTool:
    def get_document_history(did: str | None = None) -> list[dict[str, Any]] | dict[str, str]:
        try:
            ...
            return [entry.model_dump(by_alias=True, exclude_none=True) for entry in history]
        except Exception as error:
            return _structured_error(error)

    return StructuredTool.from_function(
        func=get_document_history,
        name=_with_prefix(tool_prefix, "get_document_history"),
...
def create_agent_did_tools(
    *,
    ...
    if expose.current_identity:
        tool_builders.append(_build_get_current_identity_tool(runtime_identity, tool_prefix))
    if expose.resolve_did:
        tool_builders.append(_build_resolve_did_tool(runtime_identity, tool_prefix))
    if expose.verify_signatures:
        tool_builders.append(_build_verify_signature_tool(runtime_identity, tool_prefix))
    if expose.sign_payload:
        tool_builders.append(_build_sign_payload_tool(agent_identity, runtime_identity, tool_prefix))
    if expose.sign_http:
    ...
    if expose.document_history:
        tool_builders.append(_build_document_history_tool(runtime_identity, tool_prefix))
    if expose.rotate_keys:
        tool_builders.append(_build_rotate_keys_tool(runtime_identity, tool_prefix))
