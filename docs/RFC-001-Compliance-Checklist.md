...
| SHOULD-03 | Interoperable verification mode with external implementations and fixtures aligned to RFC-001 v0.3 | PASS | `fixtures/interop-vectors.json` (shared legacy + `did:webvh` vectors), `sdk/tests/InteropVectors.test.ts`, `sdk/tests/crypto.test.ts`, `sdk-python/tests/test_interop_vectors.py`, `sdk-python/scripts/conformance_rfc001.py`, `.github/workflows/ci-cross-sdk-interop.yml`, `sdk/tests/AgentIdentity.test.ts`, `sdk-python/tests/test_identity.py`, `sdk/src/core/AgentIdentity.ts` (verifySignature/verifyHttpRequestSignature, `resolveControllerChain`) | Maintain shared fixtures, keep the dedicated interop workflow green, and add third-party vectors as external implementations mature. |
